...
app.get("/players/:playerId/matches/", async (request, response) => {
  ...
  const getPlayerMatchesQuery = `
    ...
    WHERE player_id=${playerId};`;
  const playerMatch = await database.all(getPlayerMatchesQuery);
  response.send(playerMatch.map((eachMatch) => convertMatch(eachMatch)));
});
...
app.get("/matches/:matchId/players/", async (request, response) => {
  ...
  const getMatchPlayersQuery = `
    ...
    WHERE match_id=${matchId};`;
  const playerMatch = await database.all(getMatchPlayersQuery);
  response.send(playerMatch.map((eachPlayer) => convertPlayer(eachPlayer)));
});
